...
// ------- NFT SHOW FUNCTION.  NFT viewer module:👇-------

(function () {
    const nftTabBtn = document.getElementById('nftTabBtn');
    const cryptoTabBtn = document.getElementById('active');
    const listedArea = document.querySelector('.listed-area');
    if (!listedArea || !nftTabBtn || !cryptoTabBtn) return;
    ...
    // Ensure an #nft element exists (if you removed inline version)
    ...
    if (!nftCard) {
        ...
        // insert before the "View More" button if present, otherwise append
    ...
    // Configure your NFT images here (update with real filenames in Images/)
    const nftImages = [
        'Images/nft.png',
        'Images/core.jpg',
        'Images/pi.jpg'
    ...
    function showNftAt(i) {
        if (!nftImages || nftImages.length === 0) return;
    ...
    function showNFTView() {
        ...
        showNftAt(0);
        // update active tab styles if you manage them manually
    }
    ...
    nftTabBtn.addEventListener('click', showNFTView);
    cryptoTabBtn.addEventListener('click', showCryptoView);

    prevBtn.addEventListener('click', () => showNftAt(currentIndex - 1));
    nextBtn.addEventListener('click', () => showNftAt(currentIndex + 1));

    // keyboard support: left/right arrows when NFT view is visible
...
// Minimal total-balance mask toggle (replace/add)
// Toggles total balance (.amount-group .amount) to "$*****" when eye icon clicked.
(function initializeTotalMaskToggle() {
    const eye = document.querySelector('.balance-details img');
    const totalEl = document.querySelector('.amount-group .amount');
    if (!eye || !totalEl) return;

    let masked = localStorage.getItem('balanceMasked') === 'true';
    let originalValue = totalEl.textContent; // keeps the real formatted value

    const MASK = (originalValue.trim().startsWith('$') ? '$' : '') + '*****';

    function applyMask() {
        // keep originalValue updated before masking
        originalValue = totalEl.textContent;
        totalEl.textContent = MASK;
        eye.classList.add('active');
        masked = true;
        localStorage.setItem('balanceMasked', 'true');
    }

    function restore() {
        totalEl.textContent = originalValue;
        eye.classList.remove('active');
        masked = false;
        localStorage.setItem('balanceMasked', 'false');
    }

    // initialize according to saved preference
    if (masked) applyMask();
    ...
    // Keep originalValue in sync when totals update.
    // If masked, re-apply mask but capture new original for later reveal.
    const mo = new MutationObserver(() => {
        const current = totalEl.textContent;
        if (masked) {
            // update stored real value and keep showing mask
            originalValue = current;
            totalEl.textContent = MASK;
        } else {
            // keep stored value fresh
            originalValue = current;
        }
    });
